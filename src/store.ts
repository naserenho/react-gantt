...
// Views: Day view, Weekly view, Monthly view, Quarterly view, Yearly view
...
class GanttStore {
  ...
  @action initWidth() {
    ...
    this.viewWidth = this.width - this.tableWidth;
    // Chart width cannot be smaller than 200
  ...
  @computed get scrollLeft() {
    ...
    // The default scroll bar is in the middle
  ...
  // Content area scroll area height
  ...
  // 1px The corresponding number of milliseconds
  ...
  getMajorList(): Gantt.Major[] {
    ...
    // Iterate over time within the visible area
    while (currentDate.isBetween(translateAmp - 1, endAmp + 1)) {
      ...
      // Get the time of the next iteration
  ...
  getMinorList(): Gantt.Minor[] {
    ...
    const setEnd = (start: Dayjs) => {
      const map = {
        ...
        halfYear() {
          if (fstHalfYear.has(start.month()))
  ...
  getTaskBarThumbVisible(barInfo: Gantt.Bar) {
    ...
    return (
      barTranslateX + width < this.translateX || barTranslateX - rightSide > 0
    );
  }
  ...
  @computed get getBarList(): Gantt.Bar[] {
    ...
    const barList = flattenData.map((item, index) => {
      ...
      // The start and end dates are the same and the default is one day
